...
<title>BIST Veri Terminali V5</title>

<script src="https://telegram.org/js/telegram-web-app.js">
document.addEventListener('click', function(e){
    const card = e.target.closest('.stock');
    if(card && card.dataset && card.dataset.symbol){
        e.preventDefault();
...
document.addEventListener('touchend', function(e){
    const card = e.target.closest('.stock');
    if(card && card.dataset && card.dataset.symbol){
        if(card.dataset.touchLock === '1') return;
        card.dataset.touchLock = '1';
        setTimeout(() => card.dataset.touchLock = '0', 500);
        e.preventDefault();
        openDetail(card.dataset.symbol);
    }
}, {passive:false});
...
.stock{
    ...
    pointer-events:auto !important;
    touch-action:manipulation !important;
    -webkit-tap-highlight-color:rgba(80,120,255,.15);
    position:relative;
    z-index:2;
}
.stock:active{
...
<div class="top">
<div class="title">BIST Veri Terminali V5</div>
...
<button class="active" onclick="detailTab('depth',this)">ÖZET</button>
